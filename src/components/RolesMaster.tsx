...
export default function RolesMaster() {
  ...
  const permissionCategories: PermissionCategory[] = [
    ...
    },
    {
      ...
      permissions: [{ key: "view_notifications", label: "View Notifications" }],
    },
  ];
    ...
    try {
      ...
      if (Array.isArray(data)) {
        ...
        if (data.data && Array.isArray(data.data)) {
          ...
          rolesArray = Object.values(data).filter(
  ...
  const handleToggleActive = async (roleId: string, currentStatus: boolean) => {
    try {
      ...
      toast.success(
        `Role ${currentStatus ? "deactivated" : "activated"} successfully`,
      );
      loadRoles();
    } catch (error: any) {
                ...
                >
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex-1 min-w-0 mr-2">
                      <h3
                        className="font-semibold text-gray-800 text-base md:text-lg truncate"
                        title={role.name}
                      >
                        {role.name}
                      </h3>
                      {role.description && (
                        <p
                          className="text-xs md:text-sm text-gray-600 mt-1 line-clamp-2"
                    ...
                    <button
                      onClick={() =>
                        handleToggleActive(role.id, role.is_active)
                      }
                      className={`px-2 py-1 md:px-3 md:py-1 rounded text-xs font-medium flex-shrink-0 ${
                        role.is_active
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-50 p-2 sm:p-4">
          <div className="bg-white rounded-xl md:rounded-2xl shadow-2xl w-full max-w-full md:max-w-4xl max-h-[95vh] md:max-h-[90vh] overflow-hidden mx-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-white/90 hidden md:block">
                    {editingRole
                      ...
                      : "Define new role and assign permissions"}
                  </p>
                </div>
              </div>
              <button
            ...
            <form
              ...
              className="p-4 md:p-6 max-h-[calc(95vh-60px)] md:max-h-[calc(90vh-80px)] overflow-y-auto"
            >
              {/* Basic Information */}
                ...
                <div className="space-y-4 md:space-y-6">
                  {permissionCategories.map((category) => (
                    <div
                      ...
                      className="border border-gray-200 rounded-xl p-3 md:p-5 bg-gray-50"
                    >
                      <h5 className="font-semibold text-gray-800 mb-3 md:mb-4 text-sm md:text-lg">
                        {category.category}
                      </h5>
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
                        {category.permissions.map((perm) => (
                          <label
                            key={perm.key}
                            className={`flex items-start p-3 md:p-4 border rounded-lg cursor-pointer transition-all ${
                          ...
                          >
                            ...
                            <div className="ml-3 md:ml-4">
                              ...
                              {perm.description && (
                                <div className="text-xs md:text-sm text-gray-500 mt-1 md:mt-1.5">
                                  {perm.description}
                                </div>
                              )}
                            </div>
              ...
              <div className="mb-6 md:mb-8 p-3 md:p-4 bg-blue-50 border border-blue-200 rounded-xl">
                ...
                <p className="text-xs md:text-sm text-blue-600">
                  Selected {Object.values(permissions).filter(Boolean).length}{" "}
                  out of{" "}
