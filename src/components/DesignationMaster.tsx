  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (errorMessage.includes("role is not assigned")) {
        toast.error(
          "Selected role is not available for this department. Please select a different role.",
        );
      } else {
  ...
  const handleDelete = async (designation: Designation) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        const errorMessage =
          errorData.error || error.message || "Failed to delete designation";
        ...
        if (isFKError) {
          ...
          if (deactivateResult.isConfirmed) {
            try {
              ...
              toast.success(
                `"${designation.name}" has been deactivated successfully`,
              );
              loadDesignations();
            } catch (toggleError: any) {
              console.error("Failed to deactivate:", toggleError);
              toast.error(
                toggleError.message || "Failed to deactivate designation",
              );
            }
          }
        } else {
      ...
      if (search) {
        ...
        return (
          ...
          (designation.department_name &&
            designation.department_name.toLowerCase().includes(searchTerm)) ||
          (designation.role_name &&
            designation.role_name.toLowerCase().includes(searchTerm))
        );
      }
      ...
      <div className="p-4 md:p-6 border-b border-gray-200">
          ...
          <div className="flex justify-between items-center md:block">
            <div>
              ...
              <p className="text-xs md:text-sm text-gray-500">
                Create and manage job titles by department and role
              </p>
            </div>
            <button
        ...
        <div className="flex justify-between items-center">
          <p className="text-sm text-gray-600">
            Showing{" "}
            <span className="font-semibold">{filteredDesignations.length}</span>{" "}
            designation{filteredDesignations.length !== 1 ? "s" : ""}
                ...
                >
                  ...
                  <div className="space-y-2 mb-4">
                    <div className="flex items-center gap-2 text-sm text-gray-600">
                      ...
                      <span
                        className="truncate"
                        title={designation.department_name || "N/A"}
                      >
                    ...
                    <div className="flex items-center gap-2 text-sm text-gray-600">
                      ...
                      <span
                        className="truncate"
                        title={designation.role_name || "N/A"}
                      >
              ...
              <div className="text-center py-8 md:py-12 border-2 border-dashed border-gray-200 rounded-xl bg-gray-50">
                ...
                <p className="text-xs md:text-sm text-gray-600 mb-4 px-4">
                  {search
                    ...
                    : 'Click "Add Designation" to create your first job title'}
                </p>
                {!search && (
                  <button
                    onClick={openCreate}
                  ...
                  <div>
                    ...
                    {form.department_id && departmentRoles.length === 0 && (
                      <p className="text-xs text-yellow-600 mt-1">
                        Loading roles or no roles available for this department
                      </p>
                    )}
                  </div>
                </div>
                ...
                {/* <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-gray-700 w-24">
                      {getHierarchyLevelLabel(form.hierarchy_level)} (
