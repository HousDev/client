...
// }: LayoutProps) {
//   // ── Auth context ──────────────────────────────────────────────────────────
//   // profile / user are updated in-place by updateProfileLocally() when the
//   // user changes their name or avatar in GeneralSettings.  Because these are
//   // React state values the whole tree re-renders automatically – no extra
//   // work needed here.
//   const { profile, user, signOut, loading: authLoading } = useAuth();
...
//   // ── Display name  ←  re-reads from profile/user on every render ──────────
//   // When GeneralSettings calls updateProfileLocally({ full_name: "…" })
//   // profile.full_name changes → this memo fires → header updates instantly.
//   const displayName = useMemo(() => {
//     return (
//       (profile && (profile as any).full_name) ||
//       (user && (user as any).full_name) ||
...
//   // ── Avatar URL  ←  re-reads from profile on every render ─────────────────
//   // When GeneralSettings calls updateProfileLocally({ avatar: "http://…" })
//   // profile.avatar changes → this value updates → the <img> re-renders.
//   const avatarUrl = (profile && (profile as any).avatar) || null;
...
//           {sidebarOpen ? (
//             <img src={Logo} alt="Nayash Group" className="h-16 w-auto object-contain" style={{ filter: "brightness(0) invert(1)" }} />
//           ) : (
//             <img src={Logo} alt="N" className="h-10 w-10 object-contain" style={{ filter: "brightness(0) invert(1)" }} />
//           )}
//         </div>
...
//             <div className="flex items-center justify-between gap-3 px-3 py-2 bg-[#3D3D3D] rounded-lg">
//               <div className="flex items-center gap-3">
//                 {/* avatarUrl is live – updates when GeneralSettings uploads new avatar */}
//                 {avatarUrl ? (
...
//                   ) : (
//                     <>
//                       {/* displayName is live – updates when GeneralSettings saves new name */}
