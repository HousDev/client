...
export default function SubmitExpenseModal({
    ...
    const fetchEmployeeData = async () => {
      ...
      try {
        ...
        if (profile?.employee_id) {
          foundEmployee =
            employees.find((emp) => emp.id === profile.employee_id) || null;
          console.log(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(
        `Expense claim submitted successfully! Claim Number: ${result.claim_number}`,
      );
      ...
      const errorMessage =
        error.response?.data?.message || "Failed to submit expense claim";

      // Handle duplicate entry error
      if (
        errorMessage.includes("Duplicate entry") &&
        errorMessage.includes("claim_number")
      ) {
        toast.error(
          "A duplicate expense claim was detected. Please try again or contact support.",
        );
      } else {
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-3 mt-2">
                <p className="text-xs text-yellow-700">
                  ...
                  attach supporting documents (receipts/invoices) for
                  verification. Claims without supporting documents may be
                  rejected. Maximum file size: 5MB.
                </p>
              </div>
