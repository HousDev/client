...
export default function EditEmployeeModal({
  ...
  const handleRoleChange = useCallback(
    (roleName: string) => {
      const selectedRole =
        departmentRoles.find((role) => role.name === roleName) || // Case-sensitive
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log(
        "Updating employee with form data:",
  ...
  const displayImage =
    previewUrl ||
    (existingImage ? `${import.meta.env.VITE_API_URL}${existingImage}` : null);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-5xl min-h-[50vh] max-h-[95vh]  border border-gray-200 flex flex-col">
        ...
        <form
          ...
          className="flex flex-col flex-1 min-h-[50vh] max-h-[90vh]"
        >
                        ...
                        {showAttendanceDropdown && (
                          <div className="absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                            {officeLocations.length > 0 ? (
                              ...
                              <div className="px-3 py-2 text-sm text-gray-500">
                                No office locations available for this company
                              </div>
                            )}
                          </div>
                        )}
