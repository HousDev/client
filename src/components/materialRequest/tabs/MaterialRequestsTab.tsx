...
const MaterialRequestsTab = () => {
  ...
  const [poRequests, setPORequests] = useState<MaterialRequest[]>([]);
    ...
    try {
      ...
      if (Array.isArray(response)) {
        const filteredRequests = response.filter(
          (request: any) => request.request_type === "material",
        );

        const poRec = response.filter((pr: any) => pr.request_type === "po");
        setPORequests(Array.isArray(poRec) ? poRec : []);
        setRequests(Array.isArray(filteredRequests) ? filteredRequests : []);
  ...
  useEffect(() => {
    ...
    socket.on("new_material_request", () => {
      console.log("Socket: notifications updated");
      loadMaterialRequests();
    });

    return () => {
      socket.off("new_material_request");
    };
  }, []);
        ...
        <div className="overflow-x-auto h-[calc(100vh-160px)]">
          <table className="w-full min-w-[800px]">
            ...
            <tbody className="divide-y divide-gray-200">
              {filteredRequests.map((request: any, indx: number) => {
      ...
      <div className="md:hidden bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="overflow-x-auto h-[calc(100vh-160px)]">
          <table className="w-full min-w-[768px]">
            ...
            <tbody className="divide-y divide-gray-200">
              {filteredRequests.map((request: any, indx) => {
                const isSelected = selectedRequests.has(request.request_no);
                return (
                  <tr
                    key={indx}
