...
export default function CreateServiceOrderForm({
  ...
  const loadServices = async () => {
    try {
      ...
      setItems(
        Array.isArray(data)
          ? data.filter((d: any) => d.category === "service")
          : [],
      );
    } catch (err) {
  ...
  const getSelectedPOTypeCategory = (): string | null => {
    ...
    const selected = poTypes.find(
      (t: any) => t.id === formData.service_type_id,
    );
  ...
  const generatePONumber = async () => {
    try {
      ...
      if (res && res.so_number) {
        return res.so_number;
      }
    } catch (err) {
      console.warn(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const created: any = await ServiceOrdersApi.create(payload);
      console.log("create so response : ", created);
                ...
                <div className="flex items-center gap-2">
                  ...
                  {formData.items.length > 0 && (
                    <span className="text-xs font-medium text-gray-600 bg-gray-100 px-2 py-1 rounded-full">
                      {formData.items.length} Services
                    </span>
                  )}
                </div>
                <button
            ...
            <div className="p-4 border-b border-gray-300">
              ...
              {selectedCategory && (
                <div className="mt-2 text-xs text-[#5a5d5a]">
                  Filtering by:{" "}
                  <span className="font-medium capitalize text-[#b52124]">
                    {selectedCategory}
                  </span>
                </div>
              )}
                  ...
                  <div className="flex flex-col sm:flex-row gap-4">
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (
                          totalPercent +
                            Number(selectedPaymentTermData.percent) >
                          100
                        ) {
                          toast.error(
