...
interface Employee {
  ...
  branch: string;
  status: string;
  salary?: number | string; // Add salary field
...
const AssignCtc = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setConfigForm({
        ...configForm,
        template_id: ctcTemplatesRes.data.find((t: any) => t.is_default).id,
      });
        ...
        (emp: HrmsEmployee) => {
          ...
          if (emp.salary) {
            if (emp.salary_type === "monthly") {
              annualSalary = Number(emp.salary) * 12;
            } else if (emp.salary_type === "yearly") {
              annualSalary = Number(emp.salary);
            } else {
              // Default to daily rate
              annualSalary = Number(emp.salary) * 30 * 12;
            }
          }
            ...
            <div className="p-6 max-h-[70vh] overflow-y-auto">
              ...
              >
                  ...
                  <div className="relative">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        // Auto-fill CTC input with employee's salary
                        if (selectedEmployee && selectedEmployee.salary) {
                          setConfigForm({
                            ...configForm,
                            employee_id: selectedEmployeeId,
                            annual_ctc: selectedEmployee.salary.toString(),
                          });
                        } else {
                          setConfigForm({
                            ...configForm,
                    ...
                    >
                      ...
                      {employees.map((emp) => (
                        <option key={emp.id} value={emp.id}>
                          ...
                          {emp.salary &&
                            ` - ₹${Number(emp.salary).toLocaleString()}/year`}
                        </option>
                      ))}
                    </select>
                ...
                <div>
                  ...
                  {configForm.employee_id && configForm.annual_ctc && (
                    <p className="text-xs text-green-600 mt-1">
                      Monthly CTC: ₹
                      {(
                        parseFloat(configForm.annual_ctc) / 12
                      ).toLocaleString()}
                    </p>
                  )}
                </div>
                  ...
                  <div className="p-4">
                    <ul className="space-y-2 text-sm">
                      ...
                      <li className="flex items-start gap-2">
                        <ChevronRight className="w-3 h-3 text-yellow-600 mt-0.5 flex-shrink-0" />
                        <span className="text-gray-700">
                          The employee's current salary has been auto-filled as
                          the Annual CTC amount
                        </span>
                      </li>
                    </ul>
                  </div>
