...
const GeneralSettings: React.FC = () => {
  ...
  const loadPreferences = () => {
    ...
    if (savedSystem && isAdmin) setSystem(JSON.parse(savedSystem));
  };
  ...
  const handleLogoUpload = (file: File, type: "logo" | "favicon") => {
    // Validate file size
    const maxSize = type === "logo" ? 5 * 1024 * 1024 : 1 * 1024 * 1024; // 5MB for logo, 1MB for favicon
    if (file.size > maxSize) {
      toast.error(`File is too large. Max size is ${type === "logo" ? "5MB" : "1MB"}.`);
      return;
    }
    ...
    if (type === "logo") {
      setSystem(prev => ({ ...prev, logo: objectUrl }));
      toast.success("Logo uploaded successfully!");
    } else {
      setSystem(prev => ({ ...prev, favicon: objectUrl }));
  ...
  const handleClearLogo = () => {
    if (system.logo) {
      URL.revokeObjectURL(system.logo);
  ...
  const handleClearFavicon = () => {
    if (system.favicon) {
      URL.revokeObjectURL(system.favicon);
      setSystem(prev => ({ ...prev, favicon: null }));
      toast.info("Favicon cleared");
    }
  };

  // Tab configuration with improved styling
  const tabs = [
    ...
    ...(isAdmin ? [{ id: "system", label: "System", icon: SettingsIcon, color: "text-green-500" }] : []),
  ];
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 space-y-1">
              ...
              {tabs.map((tab) => {
                ...
                return (
                  ...
                  >
                    <div className={`p-2.5 rounded-lg ${activeTab === tab.id ? 'bg-white/20' : 'bg-gray-100 group-hover:bg-gray-200'}`}>
          ...
          <div className="flex-1">
                  ...
                  <div className="flex flex-col md:flex-row items-center md:items-start gap-8">
                    ...
                    <div className="flex-1">
                      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-6">
                        <div>
                          <h2 className="text-2xl font-bold text-gray-900">{userProfile.full_name || "User"}</h2>
                    ...
                    <div className="space-y-5">
                      ...
                      <div className="pt-4">
                        <div className="text-sm text-gray-500 bg-gray-50 rounded-lg p-4 border border-gray-100">
                          <p className="font-medium text-gray-700 mb-1">Profile Visibility</p>
                          <p className="text-sm">Your profile information is visible to team members within your organization.</p>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                  <div className="flex items-center justify-between mb-8">
                    <div>
                      ...
                      <p className="text-gray-600 mt-2">Choose how you want to receive updates and alerts</p>
                    </div>
                    <button
                      onClick={handleSaveNotifications}
                      className="px-6 py-3 bg-gradient-to-r from-[#C62828] to-[#D84343] text-white font-medium rounded-xl hover:shadow-lg transition-all"
                    >
                      <FaSave className="w-4 h-4" />
                    </button>
                  </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {[
                          ...
                          return (
                            <div key={channel.key} className={`p-5 rounded-xl border-2 transition-all ${isActive ? 'border-[#C62828] bg-[#C62828]/5' : 'border-gray-100 hover:border-gray-200'}`}>
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                  ...
                                  <div>
                                    ...
                                    <p className="text-sm text-gray-500 mt-1">Receive {channel.label.toLowerCase()} notifications</p>
                                  </div>
                                </div>
                                <button
                                  onClick={() => setNotifications(prev => ({ ...prev, [channel.key]: !prev[channel.key as keyof NotificationPreferences] }))}
                                  className={`w-12 h-6 rounded-full transition-all ${isActive ? 'bg-[#C62828]' : 'bg-gray-200'}`}
                                >
                                  <div className={`w-5 h-5 bg-white rounded-full transform transition-transform ${isActive ? 'translate-x-7' : 'translate-x-1'}`}></div>
                                </button>
                              </div>
                            </div>
                          );
                    ...
                    <div className="pt-6 border-t border-gray-100">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {[
                          { label: 'Important Updates', desc: 'Critical system notifications' },
                          { label: 'Daily Digest', desc: 'Daily summary of activities' },
                          { label: 'Weekly Reports', desc: 'Weekly performance reports' },
                          { label: 'Project Updates', desc: 'Project-related notifications' },
                        ].map((type) => (
                          <div key={type.label} className="p-4 rounded-xl border border-gray-100 hover:border-gray-200 transition-all">
                            <div className="flex items-center justify-between">
                              <div>
                  ...
                  <div className="space-y-8">
                    ...
                    <div>
                      ...
                      <form onSubmit={handlePasswordChange} className="space-y-5">
                        {[
                          { key: 'current', label: 'Current Password', value: passwordData.current },
                          { key: 'new', label: 'New Password', value: passwordData.new },
                          { key: 'confirm', label: 'Confirm Password', value: passwordData.confirm },
                        ].map((field) => (
                          <div key={field.key}>
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                              {field.label}
                            ...
                            <div className="relative">
                              ...
                              <button
                                type="button"
                                onClick={() => setShowPasswords(prev => ({ ...prev, [field.key]: !prev[field.key as keyof typeof showPasswords] }))}
                    ...
                    <div className="space-y-5">
                      ...
                      <div className="p-5 rounded-xl border border-gray-100 bg-gradient-to-r from-gray-50 to-white">
                        <div className="flex items-center justify-between">
                          ...
                          <button
                            ...
                            className={`w-12 h-6 rounded-full transition-all ${security.twoFactor ? 'bg-[#C62828]' : 'bg-gray-200'}`}
                          >
                            <div className={`w-5 h-5 bg-white rounded-full transform transition-transform ${security.twoFactor ? 'translate-x-7' : 'translate-x-1'}`}></div>
                          </button>
                        </div>
                      </div>
                      ...
                      <div className="p-5 rounded-xl border border-gray-100 bg-gradient-to-r from-gray-50 to-white">
                        <div className="flex items-center justify-between mb-4">
                          ...
                          <span className="text-lg font-bold text-gray-900">{security.sessionTimeout} min</span>
                        </div>
                        <input
                          type="range"
                          min="5"
                      ...
                      <div className="p-5 rounded-xl border border-gray-100 bg-gradient-to-r from-gray-50 to-white">
                        <div className="flex items-center gap-4 mb-4">
                          ...
                          <div>
                            <h4 className="font-semibold text-gray-900">IP Whitelist</h4>
                        ...
                        {security.ipWhitelist.length > 0 ? (
                          ...
                          <div className="text-center py-8">
                            ...
                            <p className="text-gray-500">No IP addresses added yet</p>
                          </div>
                        )}
                      </div>
                    </div>
            ...
            {/* System Tab (Admin Only) - UPDATED WITH FIXED LOGO UPLOAD */}
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
                  <div className="flex items-center justify-between mb-6">
                    <div>
                      ...
                      <p className="text-sm text-gray-500 mt-1">Configure system appearance and preferences</p>
                    </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div className="flex gap-2">
                            {[
                              ...
                              return (
                                <button
                                  key={theme.value}
                                  onClick={() => setSystem({ ...system, theme: theme.value as any })}
                                ...
                                >
                                  <Icon className={`w-5 h-5 mb-1.5 ${
                                    system.theme === theme.value ? 'text-[#C62828]' : 'text-gray-500'
                                  }`} />
                                  <span className={`text-xs font-medium ${
                                    system.theme === theme.value ? 'text-[#C62828]' : 'text-gray-700'
                                  }`}>
                                    {theme.label}
                                  </span>
                                </button>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-xs text-gray-600">Current color</p>
                              <p className="text-sm font-mono text-gray-900">{system.primaryColor}</p>
                            </div>
                          </div>
                        </div>
                      </div>
                          ...
                          <div className="border border-gray-200 rounded-lg p-4">
                            ...
                            <div className="space-y-3">
                              <div className="flex flex-col sm:flex-row gap-2">
                                ...
                                {/* Clear Button (only shows when logo exists) */}
                                {system.logo && (
                                  <button
                                    type="button"
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="border border-gray-200 rounded-lg p-4">
                            ...
                            <div className="mb-4">
                              {system.favicon ? (
                                <div className="space-y-3">
                                  <p className="text-xs text-gray-600 mb-2">Preview (16x16):</p>
                            ...
                            <div className="space-y-3">
                              <div className="flex flex-col sm:flex-row gap-2">
                                ...
                                >
                                  <FaUpload className="w-3.5 h-3.5" />
                                  {system.favicon ? "Change Favicon" : "Upload Favicon"}
                                </button>
                                
                                {/* Clear Button (only shows when favicon exists) */}
                                {system.favicon && (
                                  <button
                                    type="button"
                    ...
                    <div className="p-3 bg-blue-50 rounded-lg border border-blue-100">
                      <div className="flex items-center gap-2">
                        ...
                        <p className="text-xs text-blue-700">
                          <span className="font-medium">Note:</span> Theme and branding changes may require a refresh to take effect.
                        </p>
                      </div>
                    </div>
                  </div>
