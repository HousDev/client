...
export default function Employees({ onViewProfile }: EmployeesProps) {
  ...
  const toggleEmployeeStatus = async (id: string, currentStatus: string) => {
    try {
      ...
      const current = currentStatus || "active"; // Handle null/undefined
      ...
      // Check response - handle both response formats
      if (response) {
        // Update local state immediately for better UX
        setEmployees((prev) =>
          prev.map((emp) =>
            emp.id === id ? { ...emp, employee_status: newStatus } : emp,
          ),
        );
      ...
      // Reload employees to get correct state
      await loadEmployees();

      toast.error("Failed to update status");
    }
  };
  // ✅ ADD BULK TOGGLE FUNCTION
  const handleBulkToggleActive = async (activate: boolean) => {
    ...
    const result: any = await MySwal.fire({
      title: `${activate ? "Activate" : "Deactivate"} ${selectedItems.size} Employee(s)?`,
      text: `Are you sure you want to ${action} ${selectedItems.size} employee(s)?`,
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: activate ? "#10B981" : "#6B7280",
      cancelButtonColor: "#6b7280",
      confirmButtonText: `${activate ? "Activate" : "Deactivate"} (${selectedItems.size})`,
    });
    ...
    try {
      ...
      const promises = Array.from(selectedItems).map(async (id) => {
        try {
          await employeeAPI.updateEmployeeStatus(id, newStatus);
        } catch (err) {
          console.error(`Failed to toggle employee ${id}:`, err);
          // Revert this employee's status on error
          setEmployees((prev) =>
            prev.map((emp) =>
              emp.id === id
                ? {
      ...
      toast.success(
        `${selectedItems.size} employee(s) ${action}d successfully!`,
      );
    } catch (err) {
      console.error("Bulk toggle error:", err);
      toast.error(`Failed to ${action} some employees`);
    }
  };
    ...
    try {
      await Promise.all(
        Array.from(selectedItems).map((id) =>
          employeeAPI.deleteEmployee(Number(id)),
        ),
      );
  ...
  const loadStats = async () => {
    try {
      ...
      const active = employees.filter(
        (emp: any) => emp.employee_status === "active",
      ).length;
      const onLeave = employees.filter(
        (emp: any) => emp.employee_status === "on_leave",
      ).length;
  ...
  const filteredEmployees = employees.filter((emp) => {
    ...
    if (
      columnFilters.name &&
      !fullName.includes(columnFilters.name.toLowerCase())
    )
      return false;
    if (
      columnFilters.email &&
      !emp.email.toLowerCase().includes(columnFilters.email.toLowerCase())
    )
      return false;
    if (
      columnFilters.code &&
      !emp.employee_code
        .toLowerCase()
        .includes(columnFilters.code.toLowerCase())
    )
      return false;
    if (
    ...
    if (
      columnFilters.role &&
      !emp.role?.name?.toLowerCase().includes(columnFilters.role.toLowerCase())
    )
      return false;
    if (
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="sticky top-32 bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden md:-mt-1">
        <div className="overflow-y-auto max-h-[calc(100vh-300px)] md:max-h-[calc(100vh-280px)]">
              ...
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <td className="px-3 md:px-4 py-1">
                  <input
                    ...
                    onChange={(e) =>
                      handleColumnFilterChange("code", e.target.value)
                    }
                    className="w-full px-2 py-1 text-[9px] md:text-xs border border-gray-300 rounded focus:ring-1 focus:ring-blue-500 focus:border-transparent"
                  />
                </td>
                            ...
                            <img
                              ...
                              onError={(e) => {
                                ...
                                if (parent) {
                                  ...
                                  fallback.className =
                                    "w-full h-full flex items-center justify-center bg-blue-100";
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="space-y-2">
                    {["active", "inactive", "on_leave", "terminated"].map(
                      (status) => (
                        <div
                          key={status}
