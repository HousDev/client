...
function SearchableSelect({
  ...
  return (
    <div ref={containerRef} className="relative">
      <div
        className={`w-full flex items-center gap-2 px-3 py-3 border rounded-lg bg-white cursor-pointer ${disabled ? "opacity-50 cursor-not-allowed" : "hover:shadow-sm"
          }`}
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((opt, idx) => (
                <li
                  ...
                  className={`px-3 py-2 cursor-pointer text-sm ${idx === highlight ? "bg-blue-50" : "hover:bg-gray-50"
                    } ${opt.id === value ? "font-medium text-gray-800" : "text-gray-700"}`}
            ...
            <form onSubmit={handleSubmit} className="p-6 max-h-[calc(100vh-200px)] overflow-y-auto">
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <SearchableSelect
                      options={vendors.map(v => ({ id: v.id, name: v.name || v.vendor_name || v.display || '' }))}
                      value={formData.vendor_id}
                      onChange={(id) => setFormData({ ...formData, vendor_id: id })}
                  ...
                  <div>
                    ...
                    <SearchableSelect
                      options={projects.map(p => ({ id: p.id, name: p.name || p.project_name || '' }))}
                  ...
                  <div>
                    ...
                    <SearchableSelect
                      options={poTypes.map((t: any) => ({ id: t.id, name: t.name }))}
              ...
              <div className="mb-6">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button type="button" onClick={() => { setShowItemSelector(true); setItemSelectorSearch(""); }} className="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition text-sm flex items-center gap-2">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[60] p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[80vh] overflow-hidden">
            <div className="bg-gradient-to-r from-green-600 to-green-700 px-6 py-4 flex justify-between items-center">
              ...
              <button onClick={() => { setShowItemSelector(false); setItemSelectorSearch(""); }} className="text-white hover:bg-white hover:bg-opacity-20 rounded-lg p-2 transition"><X className="w-5 h-5" /></button>
            </div>
            ...
            <div className="p-4 border-b">
              <div className="flex gap-3 items-center">
                <div className="flex-1">
                  <input
                    placeholder={selectedCategory ? `Search ${selectedCategory} items...` : "Search all items..."}
                ...
                <div className="text-sm text-gray-600">
                  {selectedCategory ? (
                    <span>Filtering by type: <strong className="capitalize">{selectedCategory}</strong></span>
                  ) : (
            ...
            <div className="p-6 overflow-y-auto max-h-[calc(80vh-140px)]">
              <div className="grid grid-cols-1 gap-3">
                {filteredItems.length === 0 ? (
                  <div className="p-6 text-center text-gray-500">No items found for the selected PO type / search</div>
                ) : (
                  filteredItems.map((item) => (
                    <div key={item.id} onClick={() => addItemFromMaster(item)} className="p-4 border border-gray-200 rounded-lg hover:bg-blue-50 hover:border-blue-300 cursor-pointer transition">
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="font-medium text-gray-800">{item.item_name}</p>
                          <p className="text-sm text-gray-600">{item.item_code} {item.hsn_code ? `| HSN: ${item.hsn_code}` : ""}</p>
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-xs text-gray-500">GST: {item.gst_rate ?? 0}%</p>
                        </div>
                      </div>
                      <span className={`inline-block mt-2 px-2 py-1 rounded text-xs font-medium ${String(item.category).toLowerCase() === "material" ? "bg-blue-100 text-blue-700" : "bg-green-100 text-green-700"}`}>
