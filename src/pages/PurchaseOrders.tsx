...
export default function PurchaseOrders() {
  ...
  const updatePurchaseOrderStatus = async (id: string, status: string) => {
    try {
      const result: any = await MySwal.fire({
        title: status.charAt(0).toUpperCase() + status.slice(1),
        text: `Are you sure you want to ${status === "authorize"
            ? status
            : status === "approved"
              ? "approve"
              : "reject"
  ...
  // ✅ UPDATED PDF FUNCTION - Server पर काम करेगा
  const viewPoPdf = async (id: number, type = "view") => {
    try {
      setPdfLoading(true);

      // Server PDF URL
      const pdfDirectUrl = `${import.meta.env.VITE_API_URL}/pdf/po/${id}`;
      console.log("PDF URL:", pdfDirectUrl);

      if (type === "view") {
        // नई tab में PDF खोलो - सबसे reliable तरीका
        window.open(pdfDirectUrl, '_blank', 'noopener,noreferrer');
        setPdfLoading(false);
      } else {
        // Download के लिए
        const a = document.createElement("a");
      ...
      toast.error("PDF खोलने में समस्या आई");
      setPdfLoading(false);
    }
  };
  ...
  return (
    <div className="p-6">
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredPOs.map((po: any) => (
                    <tr key={po.id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`px-3 py-1 rounded-full text-xs font-medium ${po.material_status === "completed"
                              ? "bg-green-100 text-green-700"
                              : po.material_status === "partial"
                                ? "bg-yellow-100 text-yellow-700"
                                : "bg-red-100 text-red-700"
                      ...
                      <td className="px-6 py-4">
                        <div className="flex gap-2">
                          {/* ✅ UPDATED BUTTON - सीधे viewPoPdf call करेगा */}
                          <button
                            onClick={() => viewPoPdf(po.id)}
                            className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition"
                            title="View Details"
                          >
                            <FileText className="w-4 h-4" />
                          ...
                          {can("edit_pos") &&
                            ...
                            po.status !== "rejected" && (
                              <button
                                ...
                                className="p-2 text-green-600 hover:bg-green-100 rounded-lg transition"
                                title={`${(po.status === "approved" && "Authorize") ||
                                  (po.status === "draft" && "Approve")
                                  }`}
                              >
                                <FileCheck2 className="w-4 h-4" />
      ...
      {/* View Modal - अब ये use नहीं होगा क्योंकि PDF नई tab में खुलेगी */}
      {/* आप चाहें तो इसे हटा सकते हैं या comment कर सकते हैं */}
