...
export default function PayrollSummary() {
  ...
  const handleCreatePayroll = () => {
    ...
    if (existingRun) {
      toast.error(`A payroll run for ${month} ${selectedYear} already exists!`);
      return;
    }
    ...
    const payrollEmployees: PayrollEmployee[] = selectedEmployees
      .map((empId) => {
    ...
    const newRun: PayrollRun = {
      ...
      status: "processing",
      total_employees: payrollEmployees.length,
      total_amount: payrollEmployees.reduce(
    ...
    setSelectedEmployees([]);
    toast.success(
      `Payroll run created for ${month} ${selectedYear} successfully!`,
    );
  };
  ...
  return (
    <div className="space-y-5">
              ...
              <div className="flex items-center gap-1">
                <button
                  onClick={() => {
                    // Handle bulk delete
                    setPayrollRuns((runs) =>
                      runs.filter((run) => !selectedItems.has(run.id)),
                    );
                    setSelectedItems(new Set());
                    setSelectAll(false);
                    toast.success(
                      `${selectedItems.size} payroll run(s) deleted successfully!`,
                    );
                  }}
                  className="bg-red-600 hover:bg-red-700 text-white px-2 py-1 rounded text-xs font-medium transition"
                >
      ...
      {/* Main Table - Responsive with sticky header */}
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="overflow-y-auto overflow-x-auto relative max-h-[calc(100vh-295px)] md:max-h-[calc(100vh-280px)]">
          <table className="w-full min-w-[800px]">
            <thead className="sticky top-0 z-10 bg-gray-200 border-b border-gray-200">
              {/* Header Row */}
              <tr>
                ...
                filteredRuns.map((run) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 md:px-4 py-3">
                        <p className="text-xs md:text-sm text-gray-600">
                          {new Date(run.cycle_start_date).toLocaleDateString()}{" "}
                          - {new Date(run.cycle_end_date).toLocaleDateString()}
                        </p>
                      </td>
                      ...
                      {selectedRun.employees.map((emp) => (
                        <tr key={emp.id} className="hover:bg-slate-50">
                          ...
                          <td className="px-4 py-3 text-right">
                            ...
                            {emp.status === "paid" && (
                              <button className="px-3 py-1.5 text-xs border-2 border-gray-200 rounded-xl hover:bg-gray-50 hover:border-gray-300 transition-all duration-200 flex items-center justify-center gap-1 ml-auto">
                                <FileText className="h-3 w-3" />
                                Slip
                              </button>
                            )}
