...
export default function PaymentHistory() {
  ...
  const handleExport = () => {
    toast.success(
      `Exporting payment report as ${exportFormat.toUpperCase()}...`,
    );
  ...
  const handleDownloadReceipt = (payment: Payment) => {
    toast.success(
      `Downloading payment receipt for ${payment.employee_name}...`,
    );
  };
  ...
  const filteredPayments = payments.filter((payment) => {
    ...
    const matchesMonth =
      monthFilter === "" || payment.pay_month === monthFilter;
    const matchesYear =
      yearFilter === "" || payment.pay_year.toString() === yearFilter;
          ...
          <div className="bg-gradient-to-br from-gray-50 to-white rounded-2xl shadow-2xl shadow-gray-900/20 w-full max-w-md border border-gray-200 overflow-hidden relative z-10">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-lg p-3">
                <p className="text-xs text-slate-600">
                  Export will include {filteredPayments.length} payment records
            ...
            <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-xs text-slate-600">
                    {selectedPayment.employee_code}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-slate-600 mb-1">Payment Period</p>
                  <p className="font-medium text-slate-900">
                    {selectedPayment.pay_month} {selectedPayment.pay_year}
                  </p>
                </div>
              </div>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-slate-900">
                      ₹{selectedPayment.gross_amount.toLocaleString()}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-600">Total Deductions</span>
                    <span className="font-medium text-red-600">
                      -₹{selectedPayment.deductions.toLocaleString()}
                    </span>
                  </div>
                  <div className="border-t border-slate-200 pt-2 mt-2">
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold text-green-600 text-lg">
                        ₹{selectedPayment.net_amount.toLocaleString()}
                      </span>
                    </div>
                  </div>
                </div>
