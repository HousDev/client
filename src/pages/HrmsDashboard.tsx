                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-1">
                      Active: {overviewStats.activeEmployees}
                    </p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold text-blue-600 mt-2">
                      {overviewStats.activeRecruitments}
                    </p>
                  ...
                  <tbody className="divide-y divide-slate-200">
                    {recruitmentData.map((job) => (
                      <tr key={job.id} className="hover:bg-slate-50">
                        ...
                        <td className="px-6 py-4 text-sm text-slate-600">
                          {job.departments?.name || "N/A"}
                        </td>
                        <td className="px-6 py-4 text-sm text-slate-600">
                          {job.location}
                        </td>
                        <td className="px-6 py-4 text-sm text-slate-600">
                          {formatters.date(job.posted_date)}
                        </td>
                        <td className="px-6 py-4 text-sm">
                          <Badge
                            variant={
          ...
          {selectedModule === "payroll" && (
            <Card>
              <div className="p-6 border-b border-slate-200">
                ...
                <p className="text-sm text-slate-600 mt-1">
                  Recent payslips and salary information
                </p>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full">
                    ...
                    {payrollData.map((pay) => (
                      <tr key={pay.id} className="hover:bg-slate-50">
                        <td className="px-6 py-4 text-sm font-medium text-slate-900">
                          ...
                          <div className="text-xs text-slate-500">
                            {pay.employees?.employee_code}
                          </div>
                        </td>
                        <td className="px-6 py-4 text-sm text-slate-600">
                          {new Date(pay.period_month).toLocaleDateString(
                            "en-IN",
                            { month: "long", year: "numeric" },
                          )}
                        </td>
                        <td className="px-6 py-4 text-sm text-right text-slate-900">
                          {formatters.currency(pay.gross_earnings)}
                        </td>
                        <td className="px-6 py-4 text-sm text-right text-red-600">
                          {formatters.currency(pay.total_deductions)}
                        </td>
                        <td className="px-6 py-4 text-sm text-right font-semibold text-green-600">
                          {formatters.currency(pay.net_salary)}
                        </td>
                        <td className="px-6 py-4 text-sm">
                          <Badge
                            variant={
                  ...
                  <tbody className="divide-y divide-slate-200">
                    {ticketsData.map((ticket) => (
                      <tr key={ticket.id} className="hover:bg-slate-50">
                        <td className="px-6 py-4 text-sm font-medium text-slate-900">
                          {ticket.employees?.first_name}{" "}
                          {ticket.employees?.last_name}
                          <div className="text-xs text-slate-500">
                            {ticket.employees?.employee_code}
                          </div>
                        </td>
                        <td className="px-6 py-4 text-sm text-slate-600">
                          {ticket.category}
