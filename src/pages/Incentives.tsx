...
import employeeIncentiveApi from "../lib/employeeIncentiveApi";
...
export default function Incentives() {
  ...
  const loadEmployees = async () => {
    ...
    try {
      ...
      setEmployees(
        activeEmployees.map((d: any) => ({
          ...d,
          salary: calculateEmpSalary(Number(d.salary), d.salary_type),
        })),
      );
  ...
  const loadIncentives = async () => {
    setLoading(true);
    try {
      const incentiveRes: any = await employeeIncentiveApi.getAllIncentives();
      console.log(incentiveRes);
  ...
  const handleAddIncentive = async () => {
    // ✅ UPDATED: Find employee from API data instead of mockEmployees
    const selectedEmployee: any = employees.find(
    ...
    const incentiveRes: any =
      await employeeIncentiveApi.createIncentive(payload);
    if (incentiveRes.success) {
      loadIncentives();
      toast.success(incentiveRes.message);
    } else {
      toast.error(incentiveRes.message);
    }
  ...
  const handleApprove = async (id: string) => {
    try {
      const incentiveRes: any = await employeeIncentiveApi.approveIncentive(
        id,
        user.id,
      );
    ...
    toast.success("Incentive approved successfully!");
  };

  const handleReject = async (id: string) => {
    try {
      const incentiveRes: any = await employeeIncentiveApi.rejectIncentive(
        id,
        user.id,
      );
      if (incentiveRes.success) {
  ...
  const handlePay = async (id: string) => {
    try {
      const incentiveRes: any = await employeeIncentiveApi.markAsPaid(id);
      if (incentiveRes.success) {
        loadIncentives();
        toast.success(incentiveRes.message);
      } else {
  ...
  const handleDeleteIncentive = async (id: string) => {
    try {
      ...
      const incentiveRes: any =
        await employeeIncentiveApi.deleteIncentiveById(id);
      console.log("incentive res", incentiveRes);
      if (incentiveRes.success) {
        toast.success(incentiveRes.message);
        loadIncentives();
      } else {
  ...
  const getMonthName = (indx: number) => months[indx - 1];
          ...
          {selectedItems.size > 0 && (
            <div className="flex items-center gap-1 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded px-2 py-1 whitespace-nowrap shrink-0">
              ...
              <button
                onClick={() => {
                  if (
                    window.confirm(`Delete ${selectedItems.size} incentive(s)?`)
                  ) {
                    setIncentives(
                      incentives.filter((inc) => !selectedItems.has(inc.id)),
                    );
                    setSelectedItems(new Set());
                    setSelectAll(false);
                    toast.success("Incentives deleted");
                  }
                }}
              ...
              <div className="flex flex-wrap items-center gap-2 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-md px-3 py-2 w-full sm:w-auto">
                ...
                <button
                  onClick={() => {
                    if (
                      window.confirm(
                        `Are you sure you want to delete ${selectedItems.size} incentive(s)?`,
                      )
                    ) {
                      setIncentives(
                        incentives.filter((inc) => !selectedItems.has(inc.id)),
              ...
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <td className="px-3 md:px-4 py-1">
                  ...
                  >
                    ...
                    <option value="Quarterly Incentive">
                      Quarterly Incentive
                    </option>
                    <option value="Project Completion">
                      Project Completion
                    </option>
                    ...
                    >
                      ...
                      <td className="px-3 md:px-4 py-3">
                        <p className="text-xs md:text-sm text-gray-700">
                          {getMonthName(Number(incentive.month))}{" "}
                          {incentive.year}
                        </p>
                      </td>
                      ...
                      <td className="px-3 md:px-4 py-3">
                        <Badge
                          variant={getStatusColor(incentive.status)}
                          className="text-xs"
                        >
                          {incentive.status}
                        </Badge>
                      ...
                      <td className="px-3 md:px-4 py-3 relative menu-container">
                        <button
                          onClick={() =>
                            setOpenMenuId(
                              openMenuId === incentive.id ? null : incentive.id,
                            )
                          }
          ...
          <div className="bg-gradient-to-br from-gray-50 to-white rounded-2xl shadow-2xl shadow-gray-900/20 w-full max-w-2xl border border-gray-200 overflow-hidden relative z-10">
            ...
            <div className="p-6 max-h-[70vh] overflow-y-auto">
              <form
                ...
                className="space-y-4"
              >
                {/* ✅ UPDATED: Employee Select - Now fetching from API */}
                <div>
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      {employees.map((emp) => {
                        ...
                        const annualCTC =
                          parseFloat(emp.salary?.toString() || "0") * 12;
                        return (
                          <option key={emp.id} value={emp.id.toString()}>
                            {fullName} ({emp.employee_code}) - CTC: ₹
                    ...
                    <ul className="space-y-2 text-sm">
                      <li className="flex items-start gap-2">
                        ...
                        <span className="text-gray-700">
                          The incentive will be created with pending status and
                      ...
                      <li className="flex items-start gap-2">
                        <ChevronRight className="w-3 h-3 text-yellow-600 mt-0.5 flex-shrink-0" />
                        <span className="text-gray-700">
                          Approved incentives can be marked as paid once payment
                          is processed
                        </span>
                      </li>
                      <li className="flex items-start gap-2">
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-medium text-slate-900">
                      {selectedIncentive.employee_name}
                    </p>
                    <p className="text-xs text-slate-600">
                      {selectedIncentive.employee_code}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-slate-600 mb-1">Employee CTC</p>
                    <p className="font-medium text-slate-900">
                      ₹{selectedIncentive.employee_ctc.toLocaleString()}
                    </p>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Badge variant="info">
                      {selectedIncentive.incentive_type}
                    </Badge>
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-600">
                    ₹{selectedIncentive.amount.toLocaleString()}
                  </p>
                </div>
                  ...
                  <div className="border-t border-slate-200 pt-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="font-medium text-slate-900">
                          {selectedIncentive.approved_by_name}
                        </p>
                      </div>
                      <div>
                        <p className="text-sm text-slate-600 mb-1">
                        ...
                        <p className="text-slate-900">
                          {selectedIncentive.approved_date &&
                            new Date(
                              selectedIncentive.approved_date,
                            ).toLocaleDateString()}
                        </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fadeIn">
          ...
          <div className="bg-gradient-to-br from-gray-50 to-white rounded-2xl shadow-2xl shadow-gray-900/20 w-full max-w-2xl border border-gray-200 overflow-hidden relative z-10">
            ...
            <div className="border-t border-slate-200 p-4 flex gap-3">
              ...
              <button className="flex-1 bg-gradient-to-r from-[#C62828] to-red-600 text-white py-2.5 px-4 rounded-xl hover:from-red-600 hover:to-red-700 transition-all duration-200 font-semibold disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
