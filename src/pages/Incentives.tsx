...
// import { Plus, Search, Filter, Download, Award, TrendingUp, Users, India, Upload, X, CheckCircle, XCircle } from 'lucide-react';
...
// import { useState, useEffect } from 'react';
// import { Plus, Filter, Download, Award, TrendingUp, Users, Upload, X, CheckCircle, XCircle, MoreVertical, Eye, Trash2, Calendar, ChevronDown, AlertCircle, ChevronRight, FileText, Save, IndianRupee } from 'lucide-react';
// import Card from '../components/ui/Card';
// import Button from '../components/ui/Button';
// import Badge from '../components/ui/Badge';
...
export default function Incentives() {
  ...
  const handleAddIncentive = () => {
    ...
    const newIncentive: Incentive = {
      ...
      employee_ctc: parseFloat(selectedEmployee.salary?.toString() || '0') * 12, // ✅ Annual CTC from monthly salary
      incentive_type: formData.incentive_type,
      amount: parseFloat(formData.amount),
      month: formData.month,
      year: formData.year,
          ...
          <div className="bg-gradient-to-br from-gray-50 to-white rounded-2xl shadow-2xl shadow-gray-900/20 w-full max-w-2xl border border-gray-200 overflow-hidden relative z-10">
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      {employees.map(emp => {
                        ...
                        const annualCTC = parseFloat(emp.salary?.toString() || '0') * 12;
                        return (
                          <option key={emp.id} value={emp.id.toString()}>
                            {fullName} ({emp.employee_code}) - CTC: ₹{annualCTC.toLocaleString()}
                          </option>
                        );
                      })}
                    </select>
            ...
            <div className="border-t border-slate-200 p-4 flex gap-3">
              ...
              <button
                type="button"
                onClick={handleAddIncentive}
                disabled={!formData.employee_id || !formData.amount || parseFloat(formData.amount) <= 0 || !formData.description || loadingEmployees}
                className="flex-1 bg-gradient-to-r from-[#C62828] to-red-600 text-white py-2.5 px-4 rounded-xl hover:from-red-600 hover:to-red-700 transition-all duration-200 font-semibold disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                <Save className="w-5 h-5" />
                Create Incentive
