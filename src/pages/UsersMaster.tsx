...
export default function UsersMaster() {
  ...
  const { user: currentUser, updateAvatarLocally, can } = useAuth();
    ...
    async (userId: string, profilePictureUrl: string | null) => {
      ...
      try {
        ...
        const selectedLocation = officeLocations.find(
          (loc) => loc.id === formData.attendance_location_ids[0],
        );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.is_employee) {
      ...
      if (!formData.attendance_location_ids) {
    ...
    <div className="p-4 md:p-6 -mt-9 md:-mt-12 px-0 md:px-0 bg-gray-50 ">
      <div className=" sticky top-16 z-10  mt-0 mb-1 px-0 py-1 md:p-4 flex flex-col md:flex-row md:items-center justify-between gap-1 md:gap-3">
        ...
        <div className="flex items-center gap-1 md:gap-2 flex-nowrap md:flex-wrap w-full md:w-auto">
          {/* Bulk Actions */}
          {selectedUsers.size > 0 && can("user_bulk_action") && (
