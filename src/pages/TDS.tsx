...
export default function TDS() {
  ...
  const handleMarkFiled = (recordId: string) => {
    setTdsRecords((records) =>
      records.map((r) => (r.id === recordId ? { ...r, status: "filed" } : r)),
    );
  ...
  const stats = {
    totalEmployees: new Set(
      tdsRecords
        .filter((r) => r.financial_year === yearFilter)
        .map((r) => r.employee_id),
    ).size,
    totalDeducted: tdsRecords
      .filter((r) => r.financial_year === yearFilter)
      .reduce((sum, r) => sum + r.tds_deducted, 0),
    totalDeposited: tdsRecords
      .filter((r) => r.financial_year === yearFilter)
      .reduce((sum, r) => sum + r.tds_deposited, 0),
    pendingDeposit: tdsRecords.filter(
      (r) => r.financial_year === yearFilter && r.status === "pending",
    ).length,
  };
  ...
  const filteredRecords = tdsRecords.filter((record) => {
    ...
    const matchesAmount =
      searchAmount === "" || String(record.tds_deducted).includes(searchAmount);
    ...
    <div className="space-y-5">
      ...
      <div className="sticky top-20 z-20 px-2 py-2 -mt-2 -mb-2">
        ...
        <div className="flex lg:hidden items-center justify-end gap-1 flex-nowrap overflow-x-auto">
          ...
          {can("generate_form16") && (
            <Button variant="secondary" className="text-[10px] px-2 py-1 h-7">
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={8} className="px-3 md:px-4 py-8 text-center">
                    ...
                    <p className="text-gray-500 text-xs md:text-sm mt-2">
                      {searchEmployee || searchPAN || searchAmount
                        ? "Try a different search term"
                        : "No TDS records available"}
                    </p>
                  </td>
                </tr>
              ) : (
                filteredRecords.map((record) => (
                  <tr key={record.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-3 md:px-4 py-3 text-right">
                      <p className="text-xs md:text-sm text-gray-700">
                        ₹{record.taxable_income.toLocaleString()}
                      </p>
                    </td>
                    <td className="px-3 md:px-4 py-3 text-right">
                      <p className="text-xs md:text-sm font-medium text-yellow-600">
                        ₹{record.tds_deducted.toLocaleString()}
                      </p>
                    </td>
                    <td className="px-3 md:px-4 py-3">
                      <Badge
          ...
          <div className="bg-gradient-to-br from-gray-50 to-white rounded-2xl shadow-2xl shadow-gray-900/20 w-full max-w-2xl border border-gray-200 overflow-hidden relative z-10">
            ...
            <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-medium text-slate-900">
                    FY {selectedRecord.financial_year}
                  </p>
                </div>
                <div>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between">
                    <span className="text-slate-600">
                      Gross Salary (Quarterly)
                    </span>
                    <span className="font-medium text-slate-900">
                      ₹{selectedRecord.gross_salary.toLocaleString()}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-600">Taxable Income</span>
                    <span className="font-medium text-slate-900">
                      ₹{selectedRecord.taxable_income.toLocaleString()}
                    </span>
                  </div>
                  <div className="border-t border-slate-200 pt-2 mt-2">
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold text-yellow-600 text-lg">
                        ₹{selectedRecord.tds_deducted.toLocaleString()}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="border-t border-slate-200 pt-4">
                  ...
                  <div className="mt-3">
                    ...
                    <p className="font-bold text-green-600 text-lg">
                      ₹{selectedRecord.tds_deposited.toLocaleString()}
                    </p>
                  </div>
                </div>
              )}
            ...
            <div className="border-t border-slate-200 p-4 flex gap-2 justify-end">
              {selectedRecord.status === "filed" && (
                <Button
                  variant="secondary"
                  onClick={() => handleGenerateForm16(selectedRecord)}
                >
            ...
            <div className="p-6 space-y-4">
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
                <p className="text-sm text-blue-700">
                  ...
                  {selectedRecord.quarter} FY {selectedRecord.financial_year}
                  <br />
                  TDS Amount:{" "}
                  <strong>
                    ₹{selectedRecord.tds_deducted.toLocaleString()}
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setDepositForm({ ...depositForm, branch: e.target.value })
                  }
