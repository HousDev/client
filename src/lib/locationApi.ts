...
export const locationApi = {
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        if (Array.isArray(response.data)) {
          console.log(
            `Found ${response.data.length} locations in response.data`,
          );
      ...
      try {
        const testResponse = await api.get("/locations/all");
      ...
      if (error.response) {
        ...
        if (error.response.status === 404) {
          throw new Error(
            "Locations endpoint not found. Please check if backend routes are configured.",
          );
        }
      }
    ...
    try {
      ...
      if (error.response?.status === 400) {
        ...
        if (Array.isArray(errors)) {
          throw new Error(
            errors.map((err: any) => err.msg || err.message).join(", "),
          );
        }
  ...
  async getCitiesByState(state: string): Promise<string[]> {
    try {
      const response = await api.get(
        `/locations/states/${encodeURIComponent(state)}/cities`,
      );
  ...
  async getPincodesByCity(city: string): Promise<string[]> {
    try {
      const response = await api.get(
        `/locations/cities/${encodeURIComponent(city)}/pincodes`,
      );
  ...
  async getByCity(city: string): Promise<Location[]> {
    try {
      const response = await api.get(
        `/locations/city/${encodeURIComponent(city)}`,
      );
