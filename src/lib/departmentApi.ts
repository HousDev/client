...
export interface Department {
  ...
  // NEW: Added for role integration
  role_ids?: number[];
  role_names?: string[];
  manager_name?: string;
}
...
export const departmentsApi = {
  ...
  async getManagers(): Promise<Manager[]> {
    try {
      ...
      if (data && data.success && Array.isArray(data.data)) {
        ...
        console.warn("Unexpected managers API response format:", data);
        return [];
      }
    } catch (error: any) {
      console.error("Error fetching managers:", error);
  ...
  async getActive(): Promise<Department[]> {
    try {
      ...
      if (data && data.success && Array.isArray(data.data)) {
        ...
        console.warn("Unexpected active departments API response format:", data);
        return [];
      }
    } catch (error: any) {
      console.error("Error fetching active departments:", error);
  ...
  // Get available roles for department
  async getAvailableRoles(): Promise<any[]> {
    try {
      const response = await api.get(`/departments/available-roles`);
      const data = unwrap(response);
      ...
      throw new Error(
        error.response?.data?.message || error.message || "Failed to fetch available roles",
      );
    }
  },

  // Get roles by department
  async getRolesByDepartment(departmentId: string): Promise<any[]> {
    try {
      ...
      if (data && data.success && Array.isArray(data.data)) {
        ...
        console.warn("Unexpected department roles API response format:", data);
        return [];
      }
    } catch (error: any) {
      console.error("Error fetching department roles:", error);
      throw new Error(
        error.response?.data?.message || "Failed to fetch department roles",
      );
    }
  },

  // Create new department WITH roles
  ...
  async toggleActive(departmentId: string): Promise<Department> {
    try {
      const response = await api.patch(`/departments/${departmentId}/toggle`);
      const data = unwrap(response);
