...
export const expenseApi = {
  ...
  submitExpense: async (
    formData: FormData,
  ): Promise<{ id: number; claim_number: string }> => {
    try {
      console.log(
        "Submitting expense with form data:",
  ...
  getEmployeeExpenseStats: async (empId: any): Promise<ExpenseStats> => {
    try {
      const response = await api.get(`/expenses/employee-stats/${empId}`);
      return response.data.data;
    } catch (error: any) {
      console.error("Get expense stats error:", error.message);
      throw error;
